...
2. For _exact staleness_ follower reads, configure your app to use `AS OF SYSTEM TIME` with the [`follower_read_timestamp()` function](functions-and-operators.html) whenever reading from the table:
    ...
    For exact staleness reads, you can also set the `AS OF SYSTEM TIME` value for all operations in a read-only transaction:
